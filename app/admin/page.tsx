...
export default async function AdminDashboardPage() {
  ...
  allLeagues?.forEach(row => {
    if (row.league) leagueCounts[row.league] = (leagueCounts[row.league] || 0) + 1;
  });
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
        ...
        <Card className="px-4 py-3 flex items-center gap-3 border-cyan/30 bg-surface-2 shadow-lg w-full md:w-auto">
          <span className="text-xs font-mono text-muted uppercase font-bold">Motor activo:</span>
          <Badge variant="green">GPT-4o</Badge>
        </Card>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <StatsCard title="TOTAL ANALISIS" value={totalAnalyses || 0} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        <Card className="flex flex-col h-full border-border">
          ...
          <div className="space-y-2 flex-1">
            ...
            )) : <div className="text-center text-sm text-muted py-4">Sin analisis aun</div>}
          </div>
        </Card>
            ...
            {recentAnalyses?.map(a => (
              <Link key={a.id} href={`/historial/${a.id}`} className="flex justify-between items-center group bg-surface-2 p-3 rounded-xl border border-border hover:border-cyan/30 transition-colors">
                <div className="flex items-center gap-3 overflow-hidden">
                  <span className="text-xl shrink-0">{a.ai_model === 'claude' ? 'Legacy' : 'GPT'}</span>
